...
interface McpCatalogFormProps {
  ...
  footer?:
    ...
    | ((opts: {
         ...
         * validation rule â€?the footer's Save button should disable on it.
   ...
   * to 0 (no confirm bar â€?used for create mode and standalone previews).
   */
...
export function McpCatalogForm({
  ...
  // Pre-existing secret env-var keys, used to render a `â€¢â€¢â€¢â€¢â€¢â€¢â€¢â€¢` placeholder.
  ...
  const handleAuthMethodChange = (
    ...
    if (nextAuthMethod === "auth_header") {
      ...
      if (!hasAuthHeader) {
        // Bearer-token credential â€?always sensitive. The Add Header
  ...
  // Used by `isHeadersDirty`'s schema-evolution check â€?RHF's
  ...
  // credentials or redeploys the pod â€?admins must reinstall + re-enter creds.
  //
  // CAVEAT â€?react-hook-form's `dirtyFields` representation: for ARRAYS
  ...
  // produces `dirtyFields.additionalHeaders = [{...all leaves false}]` â€?  // a NON-EMPTY ARRAY that is `Boolean(...)`-truthy even though nothing
  ...
  // Env-var dirtiness alone isn't enough to require reinstall â€?adding an
  ...
  // Same shape as `isEnvDirty` â€?`additionalHeaders` array dirty is too
  // coarse on its own (adding an OPTIONAL per-install header is forward-
  // compatible â€?existing installs that don't fill it stay valid).
  ...
  const validateConfigValue = boundValidationRegex
    ...
    : undefined;
  // Already-stored static values that violate the bound environment's rule â€?  // e.g. after switching the item to a stricter environment. The add/edit
  ...
  // bar's spinner â€?track the barâ†’save phase ourselves.
    ...
    <Form {...form}>
      ...
      >
        {/* Lock fields while the confirm bar is up AND during the save
            itself â€?keeps the snapshot the bar describes in sync with
            ...
            invalid HTML â€?see comments in those table components).
                ...
                >
                  ...
                  <div className="space-y-1 text-foreground/90">
                    <div className="font-semibold text-foreground">
                      ...
                      â€œ{boundEnvironmentName}â€?                    </div>
                    <div>
                      Edit or remove{" "}
                  ...
                  render={({ field }) => {
                    const authCards: Array<{
                    ...
                    }> = [
                      {
                        ...
                        description:
                          currentServerType === "remote"
                            ? "No auth â€?server is public or single-tenant"
                            : "No auth â€?credentials passed via env vars",
                      ...
                      },
                      {
                        ...
                        description: "Legacy mode â€?kept for backwards compat",
                        icon: <KeyRound className="h-4 w-4" />,
              ...
              <div className="space-y-4">
                <div className="flex items-start justify-between gap-4">
                  <div className="space-y-1">
                    ...
                    <p className="text-sm text-muted-foreground">
                      Sent on every request â€?for tenant IDs, regions, or other
                      upstream metadata.
                    </p>
                  </div>
                  <Button
 ...
 * when nothing changed â€?so a `Boolean(dirtyFields.someArray)` check
 ...
 *
 *   - Added OPTIONAL prompted var â†?existing installs stay valid (false)
 *   - Added REQUIRED prompted var â†?existing installs missing required â†?true
 *   - Removed prompted var        â†?stored value for removed var â†?true
 *   - Type change                 â†?storage moved (e.g. plain â†?secret) â†?true
 *   - required false â†?true       â†?existing installs invalid â†?true
 *   - required true â†?false       â†?existing installs still valid â†?false
 *
 ...
 * a real catalog-spec change that the pod needs to pick up â†?true.
 *
 * Keep the two implementations in sync â€?they're the user-visible warning
...
function envChangeRequiresReinstall(prev: unknown, next: unknown): boolean {
  ...
  for (const [key, prevVal] of prevMap) {
    ...
    // Mirror of backend `promptedEnvVarsRuntimeChanged` â€?without
 ...
 * `mcp-catalog-form.utils.ts â†?transformCatalogItemToFormValues`. Used
 ...
 *
 *   - Added OPTIONAL header  â†?existing installs stay valid (false)
 *   - Added REQUIRED header  â†?existing installs are missing it â†?true
 *   - Removed header         â†?stored value for removed field â†?true
 *   - required false â†?true  â†?installs that didn't fill it â†?true
 *   - required true â†?false  â†?installs still valid â†?false
 *   - headerName change      â†?routing changes â†?true
 *   - sensitive flag flip    â†?storage bucket moved â†?true
 *   - STATIC header `value`  â†?that's the actual runtime header sent on
 *     the wire (form writes it into `userConfig[field].default` when
 *     promptOnInstallation is false). Change there â†?installs would
 *     keep sending the old value â†?true
 *
 * Deliberately ignored:
 *   - `value` on prompted headers (becomes `default` in userConfig) â€? *     just a placeholder shown at install time, doesn't affect what's
 *     actually sent
 *   - `includeBearerPrefix` (becomes `valuePrefix: "Bearer "`) â€?cosmetic
 *     wire-format detail; doesn't move storage
 *   - `description`, `title` â€?pure metadata
 *
 * Keep in sync with `backend/src/services/mcp-reinstall.ts` â€?a
...
function additionalHeadersChangeRequiresReinstall(
  ...
  for (const [key, p] of prevMap) {
    ...
    // when the header is fully static (no install prompt) â€?for prompted
